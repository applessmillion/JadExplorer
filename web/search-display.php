...
<table align="center" width="780" style="background:#efefef">
	...
	//If the name of the asset is set, do this thing.
	if(ISSET($_GET['n'])){
		$name = $_GET["n"];
	...
	//If username is set, do this thing.
	elseif(ISSET($_GET['u'])){
		$name = $_GET["u"];
		#GET NAME FROM SEARCH TERMS#
	...
	//Used for searching assets, when more than one asset is found for the search query.
	elseif(ISSET($_GET['a'])){
		$assett = $_GET["a"];
		#GET NAME FROM SEARCH TERMS#
		$search_query = mysqli_query($con, "SELECT * FROM assets WHERE asset LIKE '%$assett%' ORDER BY asset ASC LIMIT 25");
		$search_nums = mysqli_num_rows($search_query);	

		echo '<tr><th colspan="3"><h2>Found '. $search_nums .' results for Asset #'. $assett . '</h2></th></tr>';
		echo "<tr><td><b>Asset Name</b></td><td><b>Asset Tag</b></td><td><b>Asset Owner</b></td></th></tr>";
		echo '<tr><th colspan="3"><hr style="border-color:#6D7ACE; width:85%;"></th></tr>';
